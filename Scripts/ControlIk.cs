...
public class ControlIk : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        float s2=(py-L1)/L3;
        float c2 = (py-L1) >= 0 ? Mathf.Sqrt(1-s2*s2) : -Mathf.Sqrt(1-s2*s2);
        ...
        if (!float.IsNaN(theta1))
            joint_1.transform.localEulerAngles = new Vector3(0, theta1,0);
        if (!float.IsNaN(theta2))
            joint_3.transform.localEulerAngles = new Vector3(0, 0, -theta2);
    }
}
